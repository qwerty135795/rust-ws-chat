...
pub async fn register(State(pool):State<Arc<Pool<SqliteConnectionManager>>>,
    ...
    let user = User::new(&user.username, &user.email, &pbkdf_hash,
                         &salt);
...
#[axum_macros::debug_handler]
pub async fn login(State(pool):State<Arc<Pool<SqliteConnectionManager>>>,
Json(login):Json<Login>) -> axum::response::Response {
    let res = tokio::spawn(async move {
        let conn = pool.get().unwrap();
        match conn.query_row("SELECT * from users \
        where username = ?1",[login.username], |row| {
            Ok::<User, _>(User {
                id: row.get("id")?,
            ...
            Ok(user) => {
                let n_iter = NonZeroU32::new(100_000).unwrap();
                match pbkdf2::verify(pbkdf2::PBKDF2_HMAC_SHA512,n_iter,&user.salt,
                login.password.as_bytes(), &user.password_hash) {
                    Ok(_) => {
                    let claims = Claims {
                        username: user.username,
                        id: user.id.unwrap().to_owned(),
                        exp: (Utc::now().timestamp() + chrono::Duration::minutes(30).num_seconds()) as usize
                    };
