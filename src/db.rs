...
fn create_users_table(pool: Arc<Pool<SqliteConnectionManager>>) -> io::Result<()> {
    let conn = pool.get().expect("Error into create_users_table method");
    let a = conn
        .execute(
...
pub async fn init_db(pool: Arc<Pool<SqliteConnectionManager>>) -> io::Result<()> {
    ...
    tokio::try_join!(async move { create_users_table(pool) }, async move {
        create_messages_table(pool_clone)
    })?;
    Ok(())
}
